...
> **A1 baseline 출처**: 타입 분류와 무관하게, `--intent <source>`가 주어지면 A1의 intent baseline은 in-repo 탐색이 아니라 **외부 소스**(다른 레포/Jira/Confluence/붙여넣기)로 채택한다(SKILL.md Phase 0 step1 참조).
...
- **주∪부 타입 합집합(4-1)**: 부 타입이 잡히면 활성 축 = 주 타입 ∪ 부 타입. 같은 축이 한쪽에서 `✅필수`면 **엄격한 쪽(필수·게이팅)** 을 채택한다.
- **monorepo(4-2)**: 패키지별로 위 매트릭스를 적용한다. 집계 규칙(한 패키지라도 Critical → 전체 FAIL, 점수=패키지 평균, 패키지별 소절)은 [rubric.md](rubric.md) 종합 판정 참조.
...
스코프 모드는 **타입 분류와 직교한 모디파이어**다. 타입이 *어떤 축을 켤지*를 정하면, 스코프 모드는 *그 축을 무엇에 적용할지*를 정한다.

| 모드 | 트리거 | 검증 대상 | 비고 |
|---|---|---|---|
| **full** (기본) | 플래그 없음 | 전체 repo | 기존 동작, 하위호환 |
| **diff** (PR-게이트) | `--diff [base-ref]` | `<merge-base>..HEAD` changeset | A1=변경 화면만, A3=관련 테스트, M-SEC=diff 라인 |

- diff 모드의 축별 스코핑·changeset 산정은 [SKILL.md](../SKILL.md)의 **Phase 0.5**가 정본이다.
- **조건부 모듈(M-DATA/M-DEP/M-OPS/M-API)** 은 그 신호 파일이 changeset에 포함될 때만 활성. 미포함이면 `diff 스코프 비활성(전체 스캔 모드에서 검증)`으로 표기하고 fail로 치지 않는다.
- A1/A2/A3 코어 축은 diff 모드에서도 항상 활성이되, 변경분 슬라이스로 좁혀서 적용한다.
